...
/** @type {string} */
const CACHE_VERSION = '1789089130|3879718';
/** @type {string} */
const CACHE_PREFIX = '离谱马场-sw-cache-';
...
self.addEventListener('install', (event) => {
	self.skipWaiting(); // liliupu: new build takes over immediately
	event.waitUntil(caches.open(CACHE_NAME).then((cache) => cache.addAll(CACHED_FILES)));
});

self.addEventListener('activate', (event) => {
	self.clients.claim(); // liliupu
